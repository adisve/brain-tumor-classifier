...
def _train_model(arch, image_gen, X_train, y_train, X_val, y_val, model, config):
    ...
    early_stopping, scheduler = init_earlystopping_and_scheduler(
        config['min_delta'], config['patience'], config['lr_factor'])
...
def build_and_compile_model_custom(num_conv_blocks, num_dense_layers, config, input_shape=(224, 224, 3)):
    ...
    model.add(Dense(4, activation='softmax',
              kernel_regularizer=l2(config['regularizer_value'])))
...
def build_and_compile_model(architecture, config, metrics=['accuracy', 'AUC'], image_size=224):
    ...
    # Since we are using pretrained weights we do not need to specify an initializer like GlorotUniform in our layers
    x = Dense(4, activation='softmax', kernel_regularizer=l2(
        config['regularizer_value']))(x)
...
def train_model_with_kfold(model, arch, X_train, y_train, image_gen, config, n_splits=5) -> List:
    ...
    for train_index, val_index in kf.split(X_train):
        ...
        history = _train_model(arch, image_gen, train_X,
                               train_y, val_X, val_y, model, config)
    ...
    print(
        f"Averaged metrics for {arch}: Loss: {avg_metrics[0]}, Accuracy: {avg_metrics[1]}, AUC: {avg_metrics[2]}")
...
def train_models_with_kfold(models, X_train, y_train, image_gen, config, n_splits=5) -> Dict:
    ...
    for arch, model in models.items():
        metrics_list = train_model_with_kfold(
            model, arch, X_train, y_train, image_gen, config, n_splits)

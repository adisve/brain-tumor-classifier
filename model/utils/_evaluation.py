...
def generate_single_report(model_name, metrics_list, model, X_test, y_test) -> None:
...
def evaluate_models(models, X_test, y_test) -> None:
    for model_name, model in models.items():
        _evaluate_model(model, X_test, y_test, model_name)

def _evaluate_model(model, X_test, y_test, model_name) -> None:
    predicted_classes = np.argmax(model.predict(X_test), axis=1)
